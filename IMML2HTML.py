...
def CleanMarkup(markup, urlString, tag):
	...
	Return "`" + markup + urlString + "`" or "" if nothing left
	"""
	result = ""
	chars = set("0123456789+-" + tag)
	i = 0
	for i in range(len(markup)):
		if markup[i] not in chars:
			result += markup[i]
...
def ProcessPropMono(bLine):
	cLine = ""
	inMarkup = False                    # we're actually processing markup commands
	sawEscape = False                    # have seen opening "`", need to look at next character
	inMono = False
	markup = ""                         # contains the markup characters up to a "=" (in URL)
	urlStr = ""                         # if present, contains "=" and the URL
	markupchars = set("mg+-0123456789pbiu")
	# print bLine
	for c in bLine:                        # character by character scan of the line
		...
		if sawEscape:                 # we have seen the "`" and need to look at this character
			sawEscape = False         # we will handle it either way
			d = c.lower()
			if d in markupchars:     # it's legitimate markup
				# print "In Markup: '%s', line is: '%s'" % (d, cLine)
				inMarkup = True         # initialize the markup stuff
				markup = c
				urlStr = ""
			else:                     # Not really markup - the "`" was just escaping this character
				cLine += "`" + c     # and push it onto the output stream
				# print "Found Escape: '%s', line is '%s'" % (d, cLine)
		elif c == "`":                    # start/end markup section
			if inMarkup:                # if we're already within markup
				# print "in Markup: '%s', '%s'" % (c, markup)
			...
			else:                          # we've seen the opening "`"
				# print "saw escape:"
				sawEscape = True          # Remember we we've seen the escape
	...
	# end of the character-by-character loop
	if markup != "":
...
def IMMLtoHTML(aLine):
	...
	bLine = bLine.replace("&","&amp;")
	bLine = bLine.replace("<","&lt;")           # convert < and > to HTML entities before putting in HTML
	bLine = bLine.replace(">","&gt;")
	bLine = bLine.replace("``","&#92;")
